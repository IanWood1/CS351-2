...
//==============================================================================  
  ...
  if(myRunMode>1) {									// 0=reset; 1= pause; 2=step; 3=run
		...
		{		// bounce on floor
			zvelNow = -zvelNow;
			console.log("flipped Zvel:", zvelNow);
		}
  ...
  gl.drawArrays(gl.LINES,  1, 24 + g_numfloorverts);
...
// Calculate the size of vertex array buffer
 ...
 var vertSize = s1.length + boxVerts.length + floorVerts.length;
 ...
 for(i = 0; i < floorVerts.length; i++){
	 myVerts[i+MAXVAR+boxVerts.length] = floorVerts[i];
}
