...
g_depth = 1
var g_light1_pos = vec4.fromValues(3, -3, 1, 1); // light position
var g_light2_pos = vec4.fromValues(-2, -4, 1, 1);
var g_is_on = [true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true];

function incRecursion() {
  g_depth += 1;
  document.getElementById("RecursionReport").innerHTML = g_depth;
}

function decRecursion() {
  g_depth -= 1;
  document.getElementById("RecursionReport").innerHTML = g_depth;
}
...
function decZ() {
  g_light1_pos[2] -= 1;
  g_light1_pos[2] <= 0 ? g_light1_pos[2] = 1 : g_light1_pos[2];
  gl.viewport(0,														// Viewport lower-left corner
  0,														// (x,y) location(in pixels)
  gl.drawingBufferWidth/2, 			// viewport width, height.
  gl.drawingBufferHeight);
...
//-----------Ray Tracer Objects:---------------
var g_myPic = new CImgBuf(512/2,512/2); // Create a floating-point image-buffer 
                        // object to hold the image created by 'g_myScene' object.
                        // WebGL 1.0 texture maps  require power-of-two sizes
...
var g_isJitter = 0;			// 0 == NO jittering; 1 == jittered sampling
...
//  test_glMatrix();	// make sure that the fast vector/matrix library we use
  ...
  g_myScene.initScene();       // initialize our ray-tracer (to default scene)
  ...
  drawAll();
  //---------OPTIONAL: do all the things done by 't' key; make our initia
...
*/
  ...
  console.log('1):\n var myV4 = vec4.fromValues(1,8,4,7);  result:\n', myV4);
	...
	console.log('\nMake a translate matrix to find out:\n',
	            'var transv3 = vec3.fromValues(6,7,8);\n');
...
//  by browserResize() whenever our browser window size changes; or
...
//=============================================================================
  ...
  g_isJitter = g_isJitter == 0 ? 1 : 0;	// toggle g_isJitter
