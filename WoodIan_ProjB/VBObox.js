...
//    gl_Position = a_Position;
 ...
 `precision mediump float;          // req'd for floats in frag shader
  //
  varying vec4 v_colr;
  void main() { 
  	 	 gl_FragColor = v_colr; 
...
//------------Add more shapes:
  ...
  this.bgnSphere1 = this.vboVerts;  // and the starting vertex for 'sphere1'
  this.appendWireSphere();             // (see fcn below)
  this.bgnSphere2 = this.vboVerts;  // and the starting vertex for 'sphere2'
  this.appendWireSphere();             // (see fcn below)
  this.bgnSphere3 = this.vboVerts;  // and the starting vertex for 'sphere3'
  this.appendWireSphere();             // (see fcn below)
  this.bgnCube = this.vboVerts;     // and the starting vertex for 'cube'
  this.appendWireCube();             // (see fcn below)
  this.bgnCyl = this.vboVerts;      // and the starting vertex for 'cylinder'
  this.appendWireCylinder();             // (see fcn below)
  this.bgnCone = this.vboVerts;     // and the starting vertex for 'cone'
  this.appendWireCone();             // (see fcn below)
/*
 ...
 this.appendCube();                 // YOU write these!
...
// radius 'rad'*1.048 in the xy plane centered at world-space origin (x=y=z=0)
...
// NOTE: use gl.drawArrays(gl.GL_LINES,...) to draw these vertices.
  ...
  //Set # of lines in grid--------------------------------------
	...
	this.xCount = 11;			// # of lines of constant-x to draw to make the grid 
	this.yCount = 11;		  // # of lines of constant-y to draw to make the grid 
	                        // xCount, yCount MUST be >1, and should be odd.
	                        // (why odd#? so that we get lines on the x,y axis)
...
// Draw these vertices with with gl.GL_LINES primitive.
  // cube centered at teh origin with vertices at +/- 1.0
  // create a line between each corner of the cube:
  ...
  var tmp = new Float32Array(this.vboContents.length + newVerts.length);
  tmp.set(this.vboContents, 0);  // copy old VBOcontents into tmp array.
  tmp.set(newVerts, this.vboContents.length); // copy newVerts into tmp array.
  this.vboContents = tmp;   // REPLACE old vboContents with tmp
  this.vboVerts = this.vboContents.length / 8;
}
...
VBObox0.prototype.appendWireCylinder = function() {
  newVerts = []
  // go around a circle connecting the top and bottom
  for (var i = 0; i < 360; i += 10) {
    // top
    newVerts.push(Math.cos(i * Math.PI / 180))
    newVerts.push(Math.sin(i * Math.PI / 180))
    newVerts.push(1)
    newVerts.push(1)
    newVerts.push(1)
    newVerts.push(1)
  ...
  var temp = new Float32Array(this.vboContents.length + newVerts.length)
  temp.set(this.vboContents, 0)
  temp.set(newVerts, this.vboContents.length)
  this.vboVerts += newVerts.length / 8
  this.vboContents = temp
}
...
// Render current VBObox contents.
  ...
  if (g_SceneNum == 0) {
    // copy transforms done in CScene.initScene(0) :
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
      ...
      //               this.bgnSphere1 - this.bgnDisk);  
                    // number of vertices to draw on-screen.
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(1.5,-1.0, 1.0));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,3.0, 2.0));
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-3.0, 4.0, 2));
    mat4.rotate(this.mvpMat, this.mvpMat, 0.25*Math.PI, vec3.fromValues(0,0,1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(5,1, 2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCube,
                  this.bgnCyl - this.bgnCube);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(3.0, 2.0, 2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCube,
                  this.bgnCyl - this.bgnCube);
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0.0, -2.0, 0.4));
    mat4.rotate(this.mvpMat, this.mvpMat, 0.25*Math.PI, vec3.fromValues(0,1,1));
    ...
    // sphere
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,1.1));
    ...
    // sphere
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(3,0,1.1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5,0.5,1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
      this.bgnSphere1,
      this.bgnSphere2 - this.bgnSphere1);
    ...
    // sphere
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-3,0,2.1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(2, 2, 2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
      this.bgnSphere1,
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,-15.0,2.0));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(5, 1, 2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCube,
                  this.bgnCyl - this.bgnCube);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,3.0,5.0));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCube,
                  this.bgnCyl - this.bgnCube);
    ...
    // cylinder
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,1.1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(1,1,0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCyl,
                  this.bgnCone - this.bgnCyl);
    // cylinder
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(3,0,2.1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(1,2,2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCyl,
                  this.bgnCone - this.bgnCyl);
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,3.1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
      this.bgnSphere1,
      this.bgnSphere2 - this.bgnSphere1);
    ...
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,6.1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
      this.bgnSphere1,
      this.bgnSphere2 - this.bgnSphere1);
    ...
    gl.drawArrays(gl.LINES,
                  this.bgnCone,
                  this.vboVerts - this.bgnCone);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(3,0,1.1));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(2,1,1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
    this.bgnCone,
    this.vboVerts - this.bgnCone);
    ...
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,3.1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES, this.bgnSphere1, this.bgnSphere2 - this.bgnSphere1);

    mat4.copy(this.mvpMat, tmp);
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(3,0,3.1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES, this.bgnSphere1, this.bgnSphere2 - this.bgnSphere1);

    mat4.copy(this.mvpMat, tmp);
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-3,0,5.1));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES, this.bgnSphere1, this.bgnSphere2 - this.bgnSphere1);
  } else if (g_SceneNum == 4) {
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,0.6));
    ...
    gl.drawArrays(gl.LINES,
                  ...
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(2.0,2.0,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-2,10,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-5,5,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(2,7,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(4,-2,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(-5,10,0.6));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.5, 0.5, 0.5));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnSphere1,
                  this.bgnSphere2 - this.bgnSphere1);
    mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
    mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,20,2.0));
    mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(5, 1, 2));
    gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
    gl.drawArrays(gl.LINES,
                  this.bgnCube,
  ...
  mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
  mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(g_light1_pos[0], g_light1_pos[1], g_light1_pos[2]));
  mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.1, 0.1, 0.1));
  gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
  gl.drawArrays(gl.LINES,
                this.bgnSphere1,
                this.bgnSphere2 - this.bgnSphere1);
  mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
  mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(g_light2_pos[0], g_light2_pos[1], g_light2_pos[2]));
  mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.1, 0.1, 0.1));
  gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
  gl.drawArrays(gl.LINES,
                this.bgnSphere1,
  ...
  mat4.copy(this.mvpMat, tmp);      // restore world-space mvpMat values.
  mat4.translate(this.mvpMat, this.mvpMat, vec3.fromValues(0,0,10));
  mat4.scale(this.mvpMat, this.mvpMat, vec3.fromValues(0.1, 0.1, 0.1));
  gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, this.mvpMat);
  gl.drawArrays(gl.LINES,
                this.bgnSphere1,
