...
//     == 3: ground-plane grid + sphere + 3rd shape, etc.
  ...
  this.lights = [
    [
      g_light1_pos,
      vec4.fromValues(0.3, 0.3, 0.3, 1), // ambient
      vec4.fromValues(0.8, 0.8, 0.8, 1), // diffuse
      vec4.fromValues(1, 1, 1, 1), // specular
    ],
    [
      g_light2_pos,
      vec4.fromValues(0.3, 0.3, 0.3, 1), // ambient
      vec4.fromValues(0.8, 0.8, 0.8, 1),
      vec4.fromValues(1, 1, 1, 1),
    ],
    [
      vec4.fromValues(0, 0, 10, 1),
      vec4.fromValues(0.3, 0.3, 0.3, 1), // ambient
      vec4.fromValues(0.8, 0.8, 0.8, 1), // diffuse
      vec4.fromValues(1, 1, 1, 1), // specular
    ],
  ...
  switch(num) {
    case 0:     // (default scene number; must create a 3D scene for ray-tracing
      ...
      this.item.push(new CGeom(RT_GNDPLANE));   // set item[0] to ground-plane.
                                                // use default colors.
                                                // no transforms needed.
      //------                     
      // this.item.push(new CGeom(RT_DISK));         // set item[1] to 2D disk.
  	  // vec4.set(this.item[1].gapColor,  0.1,0.1,0.1,1.0); // RGBA(A==opacity) bluish gray   
  	  // vec4.set(this.item[1].lineColor, 0.1,0.6,0.1,1.0);  // muddy red
      // this.item[1].rayTranslate(0,0,4);          // move drawing axes UP to z=2;
      // this.item[1].rayScale(1.0, 1.0, 2.0);      // make it a unit disk.
      // this.item[1].rayRotate(0.50*Math.PI, 1,0,0);  // rot 90deg on x axis to face us
      // this.item[1].rayRotate(0.25*Math.PI, 0,0,1); // z-axis rotate 45deg.
      let sphere = new CGeom(RT_SPHERE);        // create a sphere object
      sphere.lineColor = vec4.fromValues(0.8, 0.2, 0.2, 1.0);
      sphere.K_ambient = vec4.fromValues(0.8, 0.2, 0.2, 1.0);
      sphere.K_diffuse = vec4.fromValues(0.8, 0.2, 0.2, 1.0);
      sphere.K_specular = vec4.fromValues(0.8, 0.2, 0.2, 1.0);
      sphere.shiny = 100.0;
      sphere.reflect = 0.3;
      this.item.push(sphere);       // Append sphere to item[] &
      ...
      this.item[iNow].rayTranslate(1.5,-1.0, 1.1);  // move rightwards (+x)
      let sphere_green = new CGeom(RT_CHECKER_SPHERE);        // create a sphere object
      sphere_green.lineColor = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      sphere_green.K_ambient = vec4.fromValues(0.2, 0.6, 0.2, 1.0);
      sphere_green.K_diffuse = vec4.fromValues(0.2, 0.6, 0.2, 1.0);
      sphere_green.K_specular = vec4.fromValues(0.2, 0.6, 0.2, 1.0);
      sphere_green.shiny = 10.0;
      sphere_green.reflect = 0.0;
      this.item.push(sphere_green);       // Append sphere to item[] &
      sphere_green.rayTranslate(-1.5,-1.0, 1.1);  // move rightwards (+x)


      let big_purple = new CGeom(RT_SPHERE);        // create a sphere object
      big_purple.lineColor = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      big_purple.K_ambient = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      big_purple.K_diffuse = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      big_purple.K_specular = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      big_purple.shiny = 10.0;
      big_purple.reflect = 0.9;
      this.item.push(big_purple);       // Append sphere to item[] &
      big_purple.rayTranslate(0.0,3.0, 2.1);  // move rightwards (+x)
      big_purple.rayScale(2.0, 2.0, 2.0);      // make it a unit disk.


      let box = new CGeom(RT_BOX);        
      box.lineColor = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      box.K_ambient = vec4.fromValues(0.2, 0.2, 0.6, 1.0);
      box.K_diffuse = vec4.fromValues(0.2, 0.2, 0.6, 1.0);
      box.K_specular = vec4.fromValues(0.2, 0.2, 0.6, 1.0);
      box.shiny = 10.0;
      box.reflect = 0.0;
      this.item.push(box);       // Append sphere to item[] &
      box.rayTranslate(-3.0,4.0, 2);  // move rightwards (+x)
      box.rayRotate(-0.25*Math.PI, 0,0,1);  // z-axis rotate 45deg.
      box.rayScale(5.0, 1.0, 2.0);


      box = new CGeom(RT_BOX); // purple
      box.lineColor = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      box.K_ambient = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      box.K_diffuse = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      box.K_specular = vec4.fromValues(0.6, 0.2, 0.6, 1.0);
      box.shiny = 10.0;
      box.reflect = 0.0;
      this.item.push(box);       // Append sphere to item[] &
      box.rayTranslate(4.0,2.0, 2);  // move rightwards (+x)


      box = new CGeom(RT_BOX); // purple
      box.lineColor = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      box.K_ambient = vec4.fromValues(0.4, 0.2, 0.5, 1.0);
      box.K_diffuse = vec4.fromValues(0.4, 0.2, 0.5, 1.0);
      box.K_specular = vec4.fromValues(0.4, 0.2, 0.5, 1.0);
      box.shiny = 10.0;
      box.reflect = 0.0;
      this.item.push(box);       // Append sphere to item[] &
      box.rayTranslate(0.0,-2.0, 0.4);  // move rightwards (+x)
      box.rayRotate(0.25*Math.PI, 0,1,1);  // z-axis rotate 45deg.
      box.rayScale(0.1, 0.1, 0.1);
      ...
      break;
    case 1:
      this.item.push(new CGeom(RT_GNDPLANE));
      var sphere1 = new CGeom(RT_SPHERE);
      sphere1.K_ambient = vec4.fromValues(0.2, 0.8, 0.8, 1.0);
      sphere1.K_diffuse = vec4.fromValues(0.2, 0.8, 0.8, 1.0);
      sphere1.K_specular = vec4.fromValues(0.2, 0.8, 0.8, 1.0);
      sphere1.shiny = 200.0;
      sphere1.reflect = 0.2;
      this.item.push(sphere1);
      sphere1.rayTranslate(0.0,0.0, 1.1);  // move rightwards (+x)

      sphere1 = new CGeom(RT_SPHERE);
      sphere1.K_ambient = vec4.fromValues(0.2, 0.3, 0.4, 1.0);
      sphere1.K_diffuse = vec4.fromValues(0.2, 0.3, 0.4, 1.0);
      sphere1.K_specular = vec4.fromValues(0.2, 0.3, 0.4, 1.0);
      sphere1.shiny = 200.0;
      sphere1.reflect = 0.2;
      this.item.push(sphere1);
      sphere1.rayTranslate(3,0.0, 1.1);  // move rightwards (+x)
      sphere1.rayScale(0.5, 0.5, 1);


      sphere1 = new CGeom(RT_SPHERE);
      sphere1.K_ambient =  vec4.fromValues(200/255, 175/255, 180/255, 1.0);
      sphere1.K_diffuse =  vec4.fromValues(200/255, 175/255, 180/255, 1.0);
      sphere1.K_specular = vec4.fromValues(200/255, 175/255, 180/255, 1.0);
      sphere1.shiny = 2.0;
      sphere1.reflect = 0.0;
      this.item.push(sphere1);
      sphere1.rayTranslate(-3,0.0, 2.1);  // move rightwards (+x)
      sphere1.rayScale(2.0, 2.0, 2.0); 


      var box2 = new CGeom(RT_BOX);
      box2.lineColor = vec4.fromValues(0.1, 0.2, 0.2, 1.0);
      box2.K_ambient = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box2.K_diffuse = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box2.K_specular = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box2.shiny = 10.0;
      box2.reflect = 0.5;
      this.item.push(box2);       // Append sphere to item[] &
      box2.rayTranslate(0.0,3.0, 2);  // move rightwards (+x)
      box2.rayScale(5.0, 1.0, 2.0);

      var box3 = new CGeom(RT_BOX);
      box3.lineColor = vec4.fromValues(0.1, 0.2, 0.2, 1.0);
      box3.K_ambient = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box3.K_diffuse = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box3.K_specular = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box3.shiny = 10.0;
      box3.reflect = 0.5;
      this.item.push(box3);       // Append sphere to item[] &
      box3.rayTranslate(0.0,-15.0, 2);  // move rightwards (+x)
      box3.rayScale(5.0, 1.0, 2.0);

      box3 = new CGeom(RT_BOX);
      box3.lineColor = vec4.fromValues(0.1, 0.2, 0.2, 1.0);
      box3.K_ambient = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box3.K_diffuse = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      box3.K_specular = vec4.fromValues(0.2, 0.5, 0.9, 1.0);
      ...
      box3.reflect = 0.0;
      this.item.push(box3);       // Append sphere to item[] &
      box3.rayTranslate(0.0,3.0, 5);  // move rightwards (+x)
    ...
    case 2:
      this.item.push(new CGeom(RT_GNDPLANE));
      // sphere 
      var cyl = new CGeom(RT_CYLINDER);
      cyl.K_ambient = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      cyl.K_diffuse = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      cyl.K_specular = vec4.fromValues(1, 1, 1, 1.0);
      cyl.shiny = 200.0;
      cyl.reflect = 0.85;
      this.item.push(cyl);
      cyl.rayTranslate(0.0,0.0, 1.1);
      cyl.rayRotate(0.5*Math.PI, 1,0,0);
      cyl.rayScale(1, 0.5, 1);

      cyl = new CGeom(RT_CYLINDER);
      cyl.K_ambient = vec4.fromValues(0.4, 0.7, 0.4, 1.0);
      cyl.K_diffuse = vec4.fromValues(0.4, 0.7, 0.4, 1.0);
      cyl.K_specular = vec4.fromValues(0.4, 0.7, 0.4, 1.0);
      cyl.shiny = 200.0;
      cyl.reflect = 0.0;
      this.item.push(cyl);
      cyl.rayTranslate(-3.0,0.0, 2.1);
      cyl.rayRotate(0.5*Math.PI, 1,0,0);
      cyl.rayScale(1, 2, 1);

      cyl = new CGeom(RT_CYLINDER);
      cyl.K_ambient = vec4.fromValues(0.4, 0.4, 0.7, 1.0);
      cyl.K_diffuse = vec4.fromValues(0.4, 0.4, 0.7, 1.0);
      cyl.K_specular = vec4.fromValues(0.1, 0.1, 0.2, 1.0);
      cyl.shiny = 10.0;
      cyl.reflect = 0.0;
      this.item.push(cyl);
      cyl.rayTranslate(3.0,0.0, 2.1);
      ...
      var top_sphere = new CGeom(RT_RING_SPHERE);
      top_sphere.K_ambient = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      top_sphere.K_diffuse = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      top_sphere.K_specular = vec4.fromValues(1, 1, 1, 1.0);
      top_sphere.shiny = 200.0;
      top_sphere.reflect = 0;
      this.item.push(top_sphere);
      top_sphere.rayTranslate(0.0,0.0,3.1);

      var top_sphere = new CGeom(RT_SPHERE);
      top_sphere.K_ambient = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      top_sphere.K_diffuse = vec4.fromValues(0.4, 0.2, 0.0, 1.0);
      top_sphere.K_specular = vec4.fromValues(1, 1, 1, 1.0);
      top_sphere.shiny = 200.0;
      top_sphere.reflect = 0.85;
      this.item.push(top_sphere);
      top_sphere.rayTranslate(0.0,0.0,6.1);
      break;
    case 3:
      this.item.push(new CGeom(RT_GNDPLANE));
      var cone = new CGeom(RT_CONE);
      cone.K_ambient = vec4.fromValues(0.8, 0.6, 0.8, 1.0);
      cone.K_diffuse = vec4.fromValues(0.4, 0.4, 0.4, 1.0);
      cone.K_specular = vec4.fromValues(0.1, 0.1, 0.0, 1.0);
      cone.shiny = 20.0;
      cone.reflect = 0.0;
      this.item.push(cone);
      cone.rayTranslate(0.0,0.0, 2.1);
      cone.rayRotate(-0.5*Math.PI, 1,0,0);
      cone.rayScale(1, 2, 1);

      cone = new CGeom(RT_CONE);
      cone.K_ambient = vec4.fromValues(0.6, 0.8, 0.6, 1.0);
      cone.K_diffuse = vec4.fromValues(0.5, 0.5, 0.0, 1.0);
      cone.K_specular = vec4.fromValues(1, 1, 1, 1.0);
      cone.shiny = 20.0;
      this.item.push(cone);
      cone.rayTranslate(-3.0,0.0, 4.1);
      cone.rayRotate(-0.5*Math.PI, 1,0,0);
      cone.rayScale(1, 4, 1);

      cone = new CGeom(RT_CONE);
      cone.K_ambient = vec4.fromValues(0.5, 0.6, 0.7, 1.0);
      cone.K_diffuse = vec4.fromValues(0.1, 0.1, 0.7, 1.0);
      cone.K_specular = vec4.fromValues(0.1, 0.1, 0.7, 1.0);
      cone.shiny = 20.0;
      this.item.push(cone);
      cone.rayTranslate(3.0,0.0, 2.1);
      cone.rayRotate(-0.5*Math.PI, 1,0,0);
      cone.rayScale(2, 2, 1);


      var shiny_sphere = new CGeom(RT_SPHERE);
      shiny_sphere.reflect = 1;
      this.item.push(shiny_sphere);
      shiny_sphere.rayTranslate(0.0,0.0,3.1);

      var red_sphere = new CGeom(RT_SPHERE);
      red_sphere.K_ambient = vec4.fromValues(0.7, 0.2, 0.2, 1.0);
      red_sphere.K_diffuse = vec4.fromValues(0.7, 0.2, 0.2, 1.0);
      red_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      red_sphere.shiny = 3.0;
      red_sphere.reflect = 0;
      this.item.push(red_sphere);
      red_sphere.rayTranslate(3.0,0.0,3.1);


      var red_sphere = new CGeom(RT_SPHERE);
      red_sphere.K_ambient = vec4.fromValues(0.7, 0.7, 0.2, 1.0);
      red_sphere.K_diffuse = vec4.fromValues(0.7, 0.7, 0.2, 1.0);
      red_sphere.K_specular = vec4.fromValues(0.7, 0.7, 0.2, 1.0);
      red_sphere.shiny = 100.0;
      red_sphere.reflect = 0;
      this.item.push(red_sphere);
      red_sphere.rayTranslate(-3.0,0.0,5.1);
      break;
    case 4:
      this.item.push(new CGeom(RT_GNDPLANE));
      var small_sphere = new CGeom(RT_SPHERE);
      small_sphere.K_ambient = vec4.fromValues(0.7, 0.2, 0.2, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.7, 0.2, 0.2, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 3.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(0.0,0.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // green
      small_sphere.K_ambient = vec4.fromValues(0.2, 0.7, 0.2, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.2, 0.7, 0.2, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 3.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(2.0,2.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // blue
      small_sphere.K_ambient = vec4.fromValues(0.2, 0.2, 0.7, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.2, 0.2, 0.7, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 50.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(-2.0,10.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // white 
      small_sphere.K_ambient = vec4.fromValues(0.7, 0.7, 0.7, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.7, 0.7, 0.7, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 100.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(-5.0,5.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // pink
      small_sphere.K_ambient = vec4.fromValues(0.7, 0.2, 0.7, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.7, 0.2, 0.7, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 30.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(2.0,7.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // orange
      small_sphere.K_ambient = vec4.fromValues(0.7, 0.5, 0.2, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.7, 0.5, 0.2, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.2, 0.2, 0.2, 1.0);
      small_sphere.shiny = 100.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(4.0,-2.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      small_sphere = new CGeom(RT_SPHERE);
      // green
      small_sphere.K_ambient = vec4.fromValues(0.5, 0.7, 0.2, 1.0);
      small_sphere.K_diffuse = vec4.fromValues(0.3, 0.5, 0.5, 1.0);
      small_sphere.K_specular = vec4.fromValues(0.9, 0.2, 0.9, 1.0);
      small_sphere.shiny = 30.0;
      small_sphere.reflect = 0;
      this.item.push(small_sphere);
      small_sphere.rayTranslate(-5.0,10.0,0.6);
      small_sphere.rayScale(0.5, 0.5, 0.5);

      var box2 = new CGeom(RT_BOX);
      box2.reflect = 1;
      this.item.push(box2);       // Append sphere to item[] &
      box2.rayTranslate(0.0,20.0, 2);  // move rightwards (+x)
      box2.rayScale(5.0, 1.0, 2.0);

      break;
    default:    // nonsensical 'num' value?
      console.log("sorry; scene", num, " not implemented. Here's scene 0:"); 
      this.init(0);   // init the default scene.
    break;
  }
}
...
//	console.log("You called CScene.makeRayTracedImage!")
  ...
  for(j=0; j< this.imgBuf.ySiz; j++) {        // for the j-th row of pixels.
      ...
      for (var m = 0; m < Math.abs(g_AAcode); m++) {
        for (var l = 0; l < Math.abs(g_AAcode); l++){
          // sub-pixel anti-aliasing
          var new_i = i + (1 / g_AAcode**2) + ((Math.random() - 0.5) * g_isJitter)  + m /  Math.abs(g_AAcode);
          var new_j = j + (1 / g_AAcode**2) + ((Math.random() - 0.5) * g_isJitter) + l /  Math.abs(g_AAcode);
        this.rayCam.setEyeRay(this.eyeRay,new_i,new_j);  // create ray for pixel (i,j)
        // DIAGNOSTIC:------------------------------------
  ...
  */
        ...
        vec4.copy(colr, this.findColor(myHit)); 
        ...
        totalColor[0] /= (g_AAcode * g_AAcode);
        totalColor[1] /= (g_AAcode * g_AAcode);
        totalColor[2] /= (g_AAcode * g_AAcode);
        // Set pixel color in our image buffer------------------------------------
        idx = (j*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        this.imgBuf.fBuf[idx   ] = totalColor[0];	
        this.imgBuf.fBuf[idx +1] = totalColor[1];
        this.imgBuf.fBuf[idx +2] = totalColor[2];
      }
  }
  this.imgBuf.float2int();		// create integer image from floating-point buffer.
}

CScene.prototype.findColor = function(myHit, refIndex) {
  var foundColor = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  if (myHit.hitNum==1 || myHit.hitNum==0) {
    for (var light_index = 0; light_index < this.lights.length; light_index++) {
      if (!g_is_on[light_index]) {
        continue;
      }
      var lightSource = this.lights[light_index][0];
      var lightAmbient = this.lights[light_index][1];
      var lightDiffuse = this.lights[light_index][2];
      var lightSpecular = this.lights[light_index][3];
      // check if we are in the shadow
      var lightRay = new CRay();
      lightRay.orig = myHit.hitPt;
      lightRay.orig[3] = 1;
      var adjust = vec4.create();
      vec4.scale(adjust, this.eyeRay.dir, 0.00001);
      vec4.subtract(lightRay.orig, lightRay.orig, adjust);
      vec4.subtract(lightRay.dir, lightSource, lightRay.orig);
      shadowHit = new CHit();
      for (var i = 0; i < this.item.length; i++) {
        this.item[i].traceMe(lightRay, shadowHit);  // trace eyeRay thru it
      }
      var distToLight = vec4.distance(lightRay.orig, lightSource);
      // apply phong shading
      var lightDir = vec4.create(); // normalized light direction
      vec4.copy(lightDir, lightRay.dir);
      lightDir[3] = 0;
      vec4.normalize(lightDir, lightDir);
      var normal = vec4.create();
      vec4.copy(normal, myHit.surfNorm);
      normal[3] = 0;
      vec4.normalize(normal, normal);
      var viewDir = vec4.create();
      vec4.scale(viewDir, this.eyeRay.dir, -1);
      viewDir[3] = 0;
      vec4.normalize(viewDir, viewDir);
      var reflected = vec4.create();
      var c = vec4.create();
      vec4.scale(c, normal, vec4.dot(lightDir, normal));
      vec4.scale(c, c, 2);
      vec4.subtract(reflected, c, lightDir);
      vec4.normalize(reflected, reflected);
      let colr = vec4.create();
      var emissive = emissive = myHit.hitGeom.K_emissive;
      var ambient = vec4.create();
      vec4.multiply(ambient, lightAmbient, myHit.hitGeom.K_ambient);
      var diffuse = vec4.create();
      var specular = vec4.create();
      shadowHit.hitPt[3] = 0;
      if ( shadowHit.hitNum == -1 || shadowHit.hitNum == 0 || Math.abs(vec4.distance(lightRay.orig, shadowHit.hitPt)) > Math.abs(distToLight)){
        var attenuation = 1 / distToLight;
        vec4.multiply(diffuse, lightDiffuse, myHit.hitGeom.K_diffuse);
        vec4.scale(diffuse, diffuse, Math.max(0, vec4.dot(lightDir, normal)));
        vec4.scale(diffuse, diffuse, attenuation);


        vec4.multiply(specular, lightSpecular, myHit.hitGeom.K_specular);
        var mult = Math.pow(Math.max(0, vec4.dot(reflected, viewDir)), myHit.hitGeom.shiny);
        vec4.scale(specular, specular, mult);
        vec4.scale(specular, specular, attenuation);
      }
      if (foundColor[0] == 0 && foundColor[1] == 0 && foundColor[2] == 0){
      ...
      // handle reflection
      if (myHit.hitGeom.reflect != 0 && g_depth > 0){
        var saved_depth = g_depth;
        g_depth--;
        var reflectedRay = new CRay();
        vec4.copy(reflectedRay.orig, myHit.hitPt);
        vec4.scale(adjust, this.eyeRay.dir, 0.00001);
        vec4.subtract(reflectedRay.orig, reflectedRay.orig, adjust);
        var reflected = vec4.create();
        var c = vec4.create();
        vec4.scale(c, normal, vec4.dot(viewDir, normal));
        vec4.scale(c, c, 2);
        vec4.subtract(reflected, c, viewDir);
        vec4.normalize(reflected, reflected);
        vec4.copy(reflectedRay.dir, reflected);
        var reflectedHit = new CHit();
        for (var i = 0; i < this.item.length; i++) {
          this.item[i].traceMe(reflectedRay, reflectedHit);  // trace eyeRay thru it
        }
        var reflectedColor = vec4.create();
        var saved_eyeraydir = vec4.create();
        vec4.copy(saved_eyeraydir, this.eyeRay.dir);
        vec4.copy(this.eyeRay.dir, reflectedRay.dir);
        vec4.copy(reflectedColor, this.findColor(reflectedHit));
        vec4.copy(this.eyeRay.dir, saved_eyeraydir);
        vec4.scale(reflectedColor, reflectedColor, myHit.hitGeom.reflect);
        vec4.scale(reflectedColor, reflectedColor, 0.4);
        vec4.scale(colr, colr, 1 - myHit.hitGeom.reflect * 0.7);
        vec4.add(colr, colr, reflectedColor);
        g_depth = saved_depth;
      }
      vec4.add(foundColor, foundColor, colr);
    }
  }
  else { // if myHit.hitNum== -1)
    vec4.copy(foundColor, this.skyColor);
  }
  return foundColor;
}